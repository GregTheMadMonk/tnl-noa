	                          ...
	                          T* device_output = 0 )
{
   ...
   while( size_reduced > cpuThreshold )
   {
      ...
      if( 2 * grid_size. x * block_size. x < size_reduced )
         grid_size. x ++;
      int bits = 1;
      while( block_size. x > 1 )
      {
         block_size. x >>= 1;
         bits <<= 1;
      }
      ...
      {
         case 512:
            //cout << "Calling kernel " << block_size. x << endl;
            tnlCUDASimpleReductionKernel5< T, operation, 512 ><<< grid_size. x, block_size, shmem >>>( size_reduced, reduction_input, device_output, dbg_array1 );
            break;
         case 256:
            //cout << "Calling kernel " << block_size. x << endl;
            tnlCUDASimpleReductionKernel5< T, operation, 256 ><<< grid_size. x, block_size, shmem >>>( size_reduced, reduction_input, device_output, dbg_array1 );
            break;
         case 128:
            //cout << "Calling kernel " << block_size. x << endl;
            tnlCUDASimpleReductionKernel5< T, operation, 128 ><<< grid_size. x, block_size, shmem >>>( size_reduced, reduction_input, device_output, dbg_array1 );
            break;
         case  64:
            //cout << "Calling kernel " << block_size. x << endl;
            tnlCUDASimpleReductionKernel5< T, operation,  64 ><<< grid_size. x, block_size, shmem >>>( size_reduced, reduction_input, device_output, dbg_array1 );
            break;
         case  32:
            //cout << "Calling kernel " << block_size. x << endl;
            tnlCUDASimpleReductionKernel5< T, operation,  32 ><<< grid_size. x, block_size, shmem >>>( size_reduced, reduction_input, device_output, dbg_array1 );
            break;
         case  16:
            //cout << "Calling kernel " << block_size. x << endl;
            tnlCUDASimpleReductionKernel5< T, operation,  16 ><<< grid_size. x, block_size, shmem >>>( size_reduced, reduction_input, device_output, dbg_array1 );
            break;
         case   8:
            //cout << "Calling kernel " << block_size. x << endl;
            tnlCUDASimpleReductionKernel5< T, operation,   8 ><<< grid_size. x, block_size, shmem >>>( size_reduced, reduction_input, device_output, dbg_array1 );
            break;
         case   4:
            //cout << "Calling kernel " << block_size. x << endl;
            tnlCUDASimpleReductionKernel5< T, operation,   4 ><<< grid_size. x, block_size, shmem >>>( size_reduced, reduction_input, device_output, dbg_array1 );
            break;
         case   2:
            //cout << "Calling kernel " << block_size. x << endl;
            tnlCUDASimpleReductionKernel5< T, operation,   2 ><<< grid_size. x, block_size, shmem >>>( size_reduced, reduction_input, device_output, dbg_array1 );
            break;
         case   1:
            tnlAssert( false, cerr << "blockSize should not be 1." << endl );
            abort();
